...
public class ModeInputPanel extends GridPane {
    ...
    private int getFiles(ListView<String> listView) { // TODO
    	...
    	int location = -1;
    	int count = 0;
    	String[] directory = this.view.model.getBoardFile().getParentFile().list();
    	for (String i: directory) {
    		listView.getItems().add(i);
    		if ( i.equals("Starter.txt")){
    			location = count;
    		}
    		count ++;
    	}
    ...
    private void selectBoard(Label selectBoardLabel, ListView<String> boardsList) { // TODO
    	...
    	File selectedBoardFile = new File(this.view.model.getBoardFile().getParentFile(), file);
    	this.view.model = new ThreeMusketeers(selectedBoardFile);
    	selectBoardLabel.setText(String.format("Current board: %s", file));
